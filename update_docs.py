...
```
ONE BODY (shared infrastructure)
│  kite_data, db, telegram, risk_manager, order_manager,
│  stop_loss_manager, ticker_service, alert_registry, logger
│
├── 🧠 BRAIN 1 — Heavy / Confluence Engine
│   • 10-indicator weighted scoring (RSI, MACD, BB, Supertrend, EMA, VWAP,
│     Volume, ADX, Stochastic, OI Change). Score range -15 to +15.
│   • 56 F&O symbols (top-50 liquid + 6 indices)
│   • Code: `signal_engine.py`, `indicators.py`, `regime_filter.py`
│   • Plan: see **ROADMAP1.md**
│
└── 🧠 BRAIN 2 — Light / Configurable RSI Strategies
    • Single-indicator strategies on NIFTY (CE/PE options)
    • All thresholds configurable from dashboard (DB-persisted)
    • Small-capital experiments (₹2,000 per strategy)
    • Code: `strategies/light_*.py`, `light_strategy_config.py` _(in progress)_
    • Plan: see **ROADMAP2.md**
```

Both brains run on the same body. **Group toggles** in System Status → Alert Control Panel let you enable/disable each brain independently. Master switch silences everything.
...
## 🗺 Roadmaps (manual files — edit anytime)

| File | Purpose |
|------|---------|
| `ROADMAP1.md` | **Heavy Brain** plan — Confluence Engine path to LIVE production (5 phases, ~3-4 months) |
| `ROADMAP2.md` | **Light Brain** plan — Configurable RSI strategies on NIFTY (5 phases, ~19 hours coding) |

These are the source of truth for what's planned. Tick boxes as you finish. Resume next session by saying "let's continue Phase X of ROADMAP[1\|2]".
