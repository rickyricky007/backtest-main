"""Overview — Zerodha Kite snapshot home page."""
...
with st.sidebar:
    ...
    if ticker_live:
        st.success("📡 Live ticker running")
    ...
    auth.render_sidebar_kite_session(key_prefix="home")
    auth.render_logout_controls(key="kite_logout_home")
...
st.title("📊 Overview")
st.caption("Zerodha Kite snapshot — live account data at a glance.")
...
if is_live:
    ...
    st.markdown(
        ...
        'border-radius:12px;font-size:12px;">⏱ Delayed ~15min '
        '(run ticker_service.py for live)</span>',
        unsafe_allow_html=True,
    )
